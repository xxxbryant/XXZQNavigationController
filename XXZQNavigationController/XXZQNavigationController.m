...
#define XXZQKeyWindow     [[UIApplication sharedApplication] keyWindow]
#define XXZQNavViewW      [UIScreen mainScreen].bounds.size.width
#define XXZQDecelerationTime (0.4)

typedef NS_ENUM(int, XXZQNavMovingState) {
    XXZQNavMovingStateStanby = 0,
    XXZQNavMovingStateDragBegan,
    XXZQNavMovingStateDragChanged,
    XXZQNavMovingStateDragEnd,
    XXZQNavMovingStateDecelerating,
};
@interface XXZQNavigationController () <UIGestureRecognizerDelegate>
...
/**
 *  存放截屏的字典 key：控制器指针字符串  value：截屏图片
 */
@property (nonatomic, strong) NSMutableDictionary *screenShotsDict;
/**
 *  移动状态
 */
@property (nonatomic, assign) XXZQNavMovingState movingState;

@end

@implementation XXZQNavigationController
...
- (void)animatePopWithScreenShot:(UIImage *)screenShot completion:(void (^ __nullable)(BOOL finished))completion
{
    ...
    self.movingState = XXZQNavMovingStateDecelerating;
    ...
    [UIView animateWithDuration:0.5
                          delay:0
         usingSpringWithDamping:self.isPopAnimated? 0.4 : 1
          initialSpringVelocity:self.isPopAnimated? 0.5 : 0
                        options:UIViewAnimationOptionCurveEaseOut
                     animations:^{
                         [self moveViewWithX:XXZQNavViewW];//最终值：黑色遮罩的alpha为0，上一个界面截图的transform为1，self.view的frame为{TSNavViewW，0，TSNavViewW，self.view.frame.size.height};
                     }
                     completion:^(BOOL finished) {
                         self.backgroundView.hidden = YES;
                         self.view.frame = (CGRect){ {0, self.view.frame.origin.y}, self.view.frame.size };//将self.view重新移动到屏幕中
                         self.movingState = XXZQNavMovingStateStanby;
...
- (void)moveViewWithX:(float)x
{
    ...
    x = MAX(MIN(x, XXZQNavViewW), 0);
    // 设置frame的x
    self.view.frame = (CGRect){ {x, self.view.frame.origin.y}, self.view.frame.size};
    // 设置黑色遮罩的透明度，范围在[0, 0.6]之间
    self.lastScreenBlackMaskView.alpha = 0.6 * (1 - (x / XXZQNavViewW));
    // 设置上一个截屏的缩放比例，范围在[0.95, 1]之间
    CGFloat scale = x / XXZQNavViewW * 0.05 + 0.95;
    self.lastScreenShotImageView.transform = CGAffineTransformMakeScale(scale, scale);
...
- (void)paningGestureReceive:(UIPanGestureRecognizer *)recoginzer
{
    ...
    if (recoginzer.state == UIGestureRecognizerStateBegan) {
        if (self.movingState == XXZQNavMovingStateStanby
            ) {
            self.movingState = XXZQNavMovingStateDragBegan;
        ...
        if (self.movingState == XXZQNavMovingStateDragBegan || self.movingState == XXZQNavMovingStateDragChanged) {
            self.movingState = XXZQNavMovingStateDragEnd;
            [self panGestureRecognizerDidFinish:recoginzer];
        }
    }//正在拖动
    else if (recoginzer.state == UIGestureRecognizerStateChanged) {
        if (self.movingState == XXZQNavMovingStateDragBegan || self.movingState == XXZQNavMovingStateDragChanged) {
            self.movingState = XXZQNavMovingStateDragChanged;
            [self moveViewWithX:[recoginzer translationInView:XXZQKeyWindow].x];
        }
    }
}
/*
...
- (void)panGestureRecognizerDidFinish:(UIPanGestureRecognizer *)panGestureRecognizer {
    // 获取手指离开时候的速率
    CGFloat velocityX = [panGestureRecognizer velocityInView:XXZQKeyWindow].x;
    // 手指拖拽的距离
    CGFloat translationX = [panGestureRecognizer translationInView:XXZQKeyWindow].x;
    // 按照一定TSDecelerationTime的衰减时间，计算出来的目标位置
    CGFloat targetX = MIN(MAX(translationX + (velocityX * XXZQDecelerationTime / 2), 0), XXZQNavViewW);
    // 是否pop
    BOOL pop = ( targetX > XXZQMinX );
    // 设置动画初始化速率为当前手指离开的速率
    CGFloat initialSpringVelocity = fabs(velocityX) / (pop ? XXZQNavViewW - translationX : translationX);
    
    self.movingState = XXZQNavMovingStateDecelerating;
    [UIView animateWithDuration:XXZQAnimationDuration
                          delay:0
         usingSpringWithDamping:self.isPopAnimated? 0.3 : 1
          initialSpringVelocity:self.isPopAnimated? initialSpringVelocity : 0
                        options:UIViewAnimationOptionCurveEaseOut
                     animations:^{
                         [self moveViewWithX:pop ? XXZQNavViewW : 0];
                     }
                     completion:^(BOOL finished) {
                         self.backgroundView.hidden = YES;
                         if ( pop ) {
